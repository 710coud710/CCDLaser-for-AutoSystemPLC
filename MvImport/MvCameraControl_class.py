...
def check_sys_and_update_dll():
    ...
    if currentsystem == 'Windows':
        ...
        possible_dll_paths = [
            # 1. Full path from environment variable (như BasicDemo)
            ...
            # 3. Relative to current file (if DLL copied to project)
            os.path.join(os.path.dirname(__file__), 'MvCameraControl.dll'),
            # 4. Project root
            os.path.join(os.path.dirname(__file__), '..', 'MvCameraControl.dll'),
            # 5. Just the name (will search in PATH)
            "MvCameraControl.dll"
        ]
        
        # Find first existing DLL
        MvCamCtrldllPath = None
        for dll_path in possible_dll_paths:
            if dll_path and os.path.exists(dll_path):
                MvCamCtrldllPath = dll_path
                print(f"[MVS] Found DLL: {MvCamCtrldllPath}")
                break
        
        # Fallback to just name if not found
        if not MvCamCtrldllPath:
            MvCamCtrldllPath = "MvCameraControl.dll"
            print(f"[MVS] DLL not found in known paths, trying: {MvCamCtrldllPath}")
        
        # CRITICAL: Add DLL directory BEFORE loading (for dependencies)
        dll_dir = os.path.dirname(MvCamCtrldllPath) if os.path.dirname(MvCamCtrldllPath) else None
        if dll_dir and os.path.exists(dll_dir):
            # Add to PATH for older Python/Windows
            current_path = os.environ.get('PATH', '')
            if dll_dir not in current_path:
                os.environ['PATH'] = dll_dir + os.pathsep + current_path
            
            # Use os.add_dll_directory for Python 3.8+ (modern way)
            if hasattr(os, 'add_dll_directory'):
                try:
                    os.add_dll_directory(dll_dir)
                    print(f"[MVS] Added DLL directory: {dll_dir}")
                except Exception as e:
                    print(f"[MVS] Warning: Could not add DLL directory: {e}")
